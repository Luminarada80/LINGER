...
print('\nPreprocessing region.txt for Homer', flush=True)
# Load and clean the peak file
peaks = pd.read_csv(os.path.join(args.sample_data_dir, "region.txt"), sep='\t', header=None)
print(f'Region file shape before cleaning: {peaks.shape}', flush=True)
...
try:
    ...
    print(f"Error running Homer command: {e}", flush=True)
