...
def summarize_ground_truth_and_trans_reg(
    ...
    with open(summary_file_path, 'w') as summary_file:
        ...
        tg_percent = round((len(set(ground_truth_df['TG'])) / len(set(original_ground_truth_df[1]))) * 100, decimal_places)
        edge_percent = round((len(ground_truth_df['Score']) / original_ground_truth_df.shape[0]) * 100, decimal_places)

        log_and_write(summary_file, f'{len(set(ground_truth_df["TF"]))}/{len(set(original_ground_truth_df[0]))} '
                                     f'ground truth TFs in TRN ({tf_percent}%)')
        log_and_write(summary_file, f'{len(set(ground_truth_df["TG"]))}/{len(set(original_ground_truth_df[1]))} '
